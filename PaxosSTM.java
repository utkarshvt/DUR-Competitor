...
public class PaxosSTM {
	 ...
	 * @param requestId
	 */
	public void createTransactionContext(RequestId requestId) {
		if(!requestIdContextMap.containsKey(requestId)) {
			requestIdContextMap.put(requestId, new TransactionContext());
		}
	}
	...
	public AbstractObject open(String objId, String txMode, RequestId requestId, 
			String objectAccessMode, boolean retry) {
		// Create the context for the request Id if it was not created before
		createTransactionContext(requestId);
...
//											(object.getVersion()-1));
	...
	public boolean updateSharedObject(TransactionContext context) {
		...
		for(Map.Entry<String, AbstractObject> entry: writeset.entrySet()) {
			...
			shObj = sharedObjectRegistry.getSharedObject(objId);
			shObj.ObjectUnlock(objId);
			System.out.println("UnLocking Object " + objId);	
		}
